...
for config in "${CONFIGURATIONS[@]}"; do
    ...
    SUBJECT=1
    run_experiment "$SUBJECT" "SQUEEZE" "${LOG_PREFIX}_${BEST_CONFIG}" \
    "$IMG_HEIGHT" "$IMG_WIDTH" "$IMG_DEPTH" "$WINDOW_SIZE" "$WINDOW_STEP" "$HILBERT_TYPE" \
    "squeeze_params_${BEST_CONFIG}.json" 0.1 60 1024 ""
done
